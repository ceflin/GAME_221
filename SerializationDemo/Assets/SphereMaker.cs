...
public class SphereMaker : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space) && numCreated < maxSpheres)
        {
            ...
            newSphere.transform.position = newSphere.transform.position + Vector3.up * 2 * numCreated;

            newSphere.GetComponent<MoveWithLateralClamp>().moveSpeed = Random.Range(3f, 7f);
